...
def messages(id):
    ...
    try:   
        contact  = db.child("users").child(person["uid"]).child("contacts").get()
        contact = json.loads(json.dumps(contact.val())) 
       
        if contact == None:
             contact  = db.child("trainers").child(person["uid"]).child("contacts").get()
             contact = json.loads(json.dumps(contact.val())) 
             print(person["uid"])
        ...
        if message != None:
            add_message_to_database(id, todayDate, message, isTrainer) 
        if isTrainer:
            messagesDict = db.child("trainers").child(person["uid"]).child("contacts").child(id).child("messages").get().val()
        else:         
            messagesDict = db.child("users").child(person["uid"]).child("contacts").child(id).child("messages").get().val() 
...
def dashboard():
    ...
    if switch == 1:
        releaseWebCam() 
    switch = 0
    if  session['is_logged_in'] and not session["is_trainer"] :
        #get today's date 
        todayDate = datetime.today().strftime('%Y-%m-%d')
        ...
        if(meal != None):
            ...
            meals = db.child('users').child(person["uid"]).child("mealTime").child(todayDate).get()
            #if there is meals 
            if meals.val() != None:
                for m in meals.each():
                    mealsAppend[m.key()] = m.val() 
            ...
            mealarr.append(meal)   
            db.child('users').child(person["uid"]).child('mealTime').child(todayDate).child(mealTime).update({"meals":mealarr})

        #update wieght
        wieght = request.form.get('wieghtInfo')    
        if wieght != None  :
            db.child('users').child(person["uid"]).child("wieght").child(todayDate).update({"wieght":wieght})
        ...
        wieght = db.child('users').child(person["uid"]).child("wieght").get()
...
def add_message_to_database(id, todayDate, message, isTrainer):
    data = {'message' : message, 'sender': person['uid'], 'receiver': id}
    if isTrainer:
        db.child("trainers").child(person["uid"]).child("contacts").child(id).child("messages").child(todayDate).update(data)   
        db.child("users").child(id).child("contacts").child(person["uid"]).child("messages").child(todayDate).update(data)
    else:
        db.child("users").child(person["uid"]).child("contacts").child(id).child("messages").child(todayDate).update(data)   
        db.child("trainers").child(id).child("contacts").child(person["uid"]).child("messages").child(todayDate).update(data)

def remove_default_message(id):
    db.child("users").child(person["uid"]).child("contacts").child(id).child("messages").child("default").remove()  
    db.child("trainers").child(id).child("contacts").child(person["uid"]).child("messages").child("default").remove()

def add_default_message(id):
    db.child("users").child(person["uid"]).child("contacts").child(id).child("messages").child("default").update({"default":""})   
    db.child("trainers").child(id).child("contacts").child(person["uid"]).child("messages").child("default").update({"default":""})
